...
import form_invoice_processing as mod


def test_missing_file_path_fails_without_touching_bridge():
    result = mod.FormInvoiceProcessing().run("")
    assert result["success"] is False


def test_nonexistent_file_fails_without_touching_bridge():
    result = mod.FormInvoiceProcessing().run("C:/definitely/not/a/real/path.png")
    assert result["success"] is False
...
def test_run_returns_response():
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as f:
        ...
        temp_path = f.name
    try:
        with patch.object(mod, "_ask_gemini", return_value='{"total": "100.00"}') as mock_call:
            result = mod.FormInvoiceProcessing().run(temp_path)
            assert result["success"] is True
            assert result["response"] == '{"total": "100.00"}'
...
if __name__ == "__main__":
    ...
    print("All form_invoice_processing self-checks passed.")
