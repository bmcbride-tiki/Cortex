...
from m365_graph_bridge import set_excel_range as _set_excel_range


class SetExcelRange:
    """Writes a cell range in an Excel workbook via Graph's Workbook API. Reuses
    m365_graph_bridge's mock logic directly (no subprocess). Mock-mode until an Azure
    AD app registration exists."""

    def run(self, file_path: str, worksheet: str, range_address: str, values: list) -> dict:
        try:
            return {"success": True, **_set_excel_range(file_path, worksheet, range_address, values)}
        except Exception as e:
            return {"success": False, "response": f"set_excel_range error: {e}"}


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(json.dumps({"success": False, "response": 'Expected one JSON payload arg: {"file_path": "...", "worksheet": "...", "range_address": "...", "values": [["a", "b"]]}'}))
        sys.exit(1)

    try:
        params = json.loads(sys.argv[1])
        result = SetExcelRange().run(
        ...
        result = {"success": False, "response": f"set_excel_range error: {e}"}
