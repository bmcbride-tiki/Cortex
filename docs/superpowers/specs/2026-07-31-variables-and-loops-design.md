...
`_run_loop_iteration(subgraph) -> str` runs one pass over the extracted private subgraph, reusing the existing `_pick_next` scheduler (`workflow_engine.py:795-838`, unchanged) and appending to the SAME `self.log`/`self.context` the top-level run uses -- so iteration steps show up in the run report exactly like any other step, and `MAX_TOTAL_STEPS` (`workflow_engine.py:103`) continues to guard the *whole run* including loop iterations for free, with no separate cap needed for Apply to Each itself. Each iteration overwrites its inner nodes' `self.context` entries in place, so internally `self.context["SomeInnerLabel"]` ends up holding the *last* iteration's value once the loop finishes -- but connection-scoping is completely unchanged by any of this: a node outside the loop's module was never wired directly to an inner node, so it still can't reference `{{SomeInnerLabel}}` (same `MissingInputError` as referencing any other non-adjacent label today). What a node *outside* the loop reads via `{{label}}` is the **container's own label**, whose value is the loop's aggregate JSON array (see `_execute_loop_container` above) -- not any single iteration's value.
